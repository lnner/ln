...
- :first-child :选取属于其父元素的首个子元素的指定选择器——父元素是重点
- :last-child :选取属于其父元素的最后一个子元素的指定选择器
- :nth-child(n) ： 匹配属于其父元素的第 N 个子元素，不论元素的类型
- :nth-last-child(n) ：选择器匹配属于其父元素的第 N 个子元素的每个元素，不论元素的类型，从最后一个子元素开始计数。
  n 可以是数字、关键词或公式
- 
...
letter-spacing属性用于定义中文字间距，所谓字间距就是字符与字符之间的空白。其属性值可为不同单位的数值，允许使用负值，默认为normal。
...
相邻兄弟选择器（Adjacent sibling selector）可选择紧接在另一元素后的元素（+下一个同级选择器），且二者有相同父元素。

## 然而css`没有`父元素选择器

要想[在CSS中实现父选择器效果](http://www.zhangxinxu.com/wordpress/2016/08/css-parent-selector/) ,可以新建个同级子元素然后模仿（冒充）父元素（可能会用到定位或浮动使得脱标），利用兄弟选择器+，来选择该子元素（由于利用了兄弟选择器和脱标，要考虑到子元素间的z-index，不然模拟父元素的子元素遮盖住用来触发的目标子元素），模拟父元素的触发效果

### checkbox or radio CSS实现选中和未选中
...
| background-position                   | 背景位置(%,px)               |
| background-attachment                 | 背景固定还是滚动(fixed)/(scroll) |
| 背景的合写（复合属性）                           |                          |
...
通过background-size设置背景图片的尺寸，就像我们设置img的尺寸一样，在移动Web开发中做屏幕适配应用非常广泛。(不能在background里连写！)
...
（层级z-index只对非静态定位元素起作用（浮动不起作用），定位的元素越往后，层级越高）
...
### 2 可包含浮动元素，但对其它形式的脱离文档流，例如定位，transform，则失效
...
| transition-property        | 规定应用过渡的 CSS 属性的名称。(例如transform,rotate...) | 3    |
